...
function renderVehiclesPage() {
  ...
  const filterHtml = `
    <div class="filter-row" style="display: flex; gap: 12px; align-items: center; margin-bottom: 16px; flex-wrap: wrap;">
      <input type="text" id="vehicle-search" placeholder="Search by vehicle number, transporter, type"
             value="${filter}"
  ...
  const formHtml = `
    <form data-form="vehicle" class="form-grid white">
      ...
      <select name="transporterSelect" id="vehicle-transporter-select" required>
        ...
        ${(state.data.transporters || []).map(t => `<option value="${t.id}">${t.firmName}</option>`).join('')}
      </select>
  ...
  const listHtml = filteredItems.length
    ? filteredItems.map(item => {
        const transporter = state.data.transporters?.find(t => t.id === item.transporterId);
  ...
  return createMainLayout('vehicles', content);
}

export { renderVehiclesPage };